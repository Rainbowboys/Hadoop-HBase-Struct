					    ...
					    var flag=true;
						   ...
						   function addDir(){
							   ...
							   var data = $("#mkdirForm").formToArray();
								$.ajax({
									type:"POST",
									url:"${pageContext.request.contextPath}/mkdirFileAction",
									data:data,
									dataType:"json",
									timeout:5000,
						   ...
						   function deldir(){
							   ...
							   if(ids.length>0){
								   layer.confirm('删除提醒',function(index){
									   $.post('${pageContext.request.contextPath}/deleteFileAction', {ids:ids.join(','),dir:dir}, function(j) {
											if (j.success) {
												location.reload();
											}else{
												alert(json.msg);
						   ...
						   function renameBut(index,name,type){
							   ...
							   $.post('${pageContext.request.contextPath}/renameFileAction', {dir:dir,name:name,rename:newname,type:type}, function(j) {
								    if (j.success) {
										location.reload();
									}else{
										alert(json.msg);
						   ...
						   function viewName(index,name){
							   ...
							   $.layer({
								    ...
								    title: false,
								    closeBtn: [0, true],
								    iframe: {src : '${pageContext.request.contextPath}/viewFileAction?dir='+dir+'&name='+encodeURI(encodeURI(name))},
								    area: ['950px', '650px']
								});
						   }
						   function downName(name){
							   var dir = $("#dir").val();
							   window.location.href="${pageContext.request.contextPath}/downloadFileAction?dir="+dir+"&name="+encodeURI(encodeURI(name));
							   /* $.layer({
								    type: 2,
								    border: [0],
								    title: false,
								    closeBtn: [0, true],
								    iframe: {src : 'downloadFileAction?dir='+dir+'&name='+name},
								    area: ['300px', '300px']
								}); */
						   }
						   function share(){
							   ...
							   if(ids.length>0){
								   $.layer({
									    ...
									    title: '选择要分享的用户',
									    closeBtn: [0, true],
									    iframe: {src : '${pageContext.request.contextPath}/getFollowUserAction?ids='+encodeURI(encodeURI(ids))+'&dir='+dir+'&types='+types},
									    area: ['860px', '400px']
									});
							   }else{
								   alert("你没有选择");
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<tbody id="listdir">
										<c:forEach items="${files}" var="entry" varStatus="sta">
										<tr>
											...
											<td><div id="edit01${sta.index}">
													<c:if test="${entry.type=='D'}">
														<a href="${pageContext.request.contextPath}/listFileAction?name=${dir}/${entry.name}">${entry.name}</a>
													</c:if>
