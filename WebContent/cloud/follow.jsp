...
<jsp:include page="/public/top.jsp"></jsp:include>
					...
					<div class="row">
						<c:forEach items="${follows}" var="entry">
							<div class="col-lg-1 col-md-1">
								<div class="thumbnail">
									<img src="${pageContext.request.contextPath}/static/assets/avatars/profile-pic.jpg" alt="...">
								</div>
	...
	<script type="text/javascript">
		function getUser() {
			...
			$.ajax({
						type : "POST",
						url : "${pageContext.request.contextPath}/getUserUserAction",
						data : data,
						dataType : "json",
						success : function(json) {
							if (json.success) {
								...
								str += '<div class="thumbnail"><img src="${pageContext.request.contextPath}/static/assets/avatars/profile-pic.jpg"></div>';
		...
		function followUser(name) {
			$.post('${pageContext.request.contextPath}/followUserAction', {
				username : name
			}, function(j) {
				if (j.success) {
					location.reload();
		...
		function unfollowUser(name) {
			$.post('${pageContext.request.contextPath}/unfollowUserAction', {
				username : name
			}, function(j) {
				if (j.success) {
					location.reload();
